...
Die \textit{Adjazenzmatrix} $\textbf{A}=(a_{ij})_{N_V \times N_V}$, gibt an, ob es eine Kante zwischen zwei Knoten gibt. Für einen ungerichteten Graphen\footnote{Werden von einem gerichteten Graph die Richtungen der Kanten entfernt, d.h. $(u,v)=\{u,v\}=(v,u)$, bleibt als "`Skelett"' ein ungerichteter Graph übrig, der die Adjazenzmatrix $\textbf{A}$ besitzt.} ist $\textbf{A}$ symmetrisch und als 
\begin{align*}
a_{ij} := \begin{cases}
...
\end{cases}
\end{align*}
definiert ZITAT DIESTELLLLLL!!!!!\\ 
Die \textit{Inzidenzmatrix} $\textbf{B}_{\text{ung}}=(b_{ik})_{N_V \times N_E}$ eines \textit{ungerichteten} Graphen $G=(V,E)$ gibt an ob eine Kante $e_k \in E$ inzident zu $v_i \in V$ ist und wird als
\begin{align*}
b_{ik} := \begin{cases}
...
\end{cases}
\end{align*}
definiert. Für einen \textit{gerichteten} Graphen $G=(V,E)$ ist $\textbf{B}_{\text{ger}}=(b_{ik})_{N_V \times N_E}$ als
\begin{align*}
b_{ik} := \begin{cases}
